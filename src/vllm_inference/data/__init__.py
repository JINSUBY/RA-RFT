...
class BaseDataset(Dataset, ABC):
    ...
    def _load_data(dataset_names, split):
        ...
        for name in dataset_names:
            ...
            if name == "train_2k5":
                print("Use train_2k5")
                data += load_train_2k5(split=split)
            if name == "tvgbench":
                print("Use tvgbench")
...
class TemporalGroundingDataset(BaseDataset):
    def __init__(
        ...
        **kwargs,
        ...
        print(f"[DEBUG] TemporalGroundingDataset initialized with preprocessed_data_path: {preprocessed_data_path}")
        if use_prepared_video:
            self.prepared_video_dirs = []
            for dataname in dataset_names:
                self.prepared_video_dirs.append(
    ...
    def _preprocess(self, itm):
        ...
        if self.preprocessed_data_path is not None:
            video_id = itm["video"].split("/")[-1].split(".")[0]
            preprocessed_dir = os.path.join(self.preprocessed_data_path, video_id)
            preprocessed_file = os.path.join(preprocessed_dir, "video_inputs.pt")
            preprocessed_kwargs = os.path.join(preprocessed_dir, "video_kwargs.json")
            import sys
            sys.stderr.write(f"[DEBUG] Looking for preprocessed file: {preprocessed_file}\n")
            sys.stderr.flush()
            if os.path.exists(preprocessed_file) and os.path.exists(preprocessed_kwargs):
                sys.stderr.write(f"[DEBUG] Loading preprocessed data from {preprocessed_file}\n")
                sys.stderr.flush()
                video_inputs = torch.load(preprocessed_file)
                with open(preprocessed_kwargs, 'r') as f:
                    utils = json.load(f)
            else:
                sys.stderr.write(f"[DEBUG] Preprocessed file not found: {preprocessed_file}, falling back to video processing\n")
                sys.stderr.flush()
                # Fallback to video processing if preprocessed file not found
                tmp = self._load_video_from_cache(itm["video"], ele)
                if tmp is not None:
                    video_inputs, utils = tmp
                else:
