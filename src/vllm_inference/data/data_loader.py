...
def load_activitynet(split="test"):
    data_root = "./dataset/finetune/activitynet/"
    data_path = f"{data_root}/activitynet_annotation/{split}.json"
    ...
    for video_id, meta_data in data.items():
        ...
        video_dirs = ["/data/activitynet_new/test", "/data/activitynet_new/trainval", "/data/activitynet_new/val_2"]
        for video_dir in video_dirs:
            for ext in ["mp4", "mkv", "webm"]:
                tmp = os.path.join(video_dir, f"{video_id}.{ext}")
...
def load_charades(split="test"):
    ...
    qid, conv_data = 0, []
    for video_id, meta_data in data.items():
        video_path = os.path.join("/data/sungjoon/Charades_v1", f"{video_id}.mp4")
        for i in range(len(meta_data["timestamps"])):
            conv_data.append(
                {
                    "video": video_path,
...
def load_train_2k5(split="train"):
    """
    Load train_2k5.json data for inference.
    This function loads data in the format already present in train_2k5.json
    and returns it ready for inference.
    """
    data_path = "./dataset/timer1/annotations/train_2k5.json"
