...
def process_split(
    ...
    # Handle different data formats: dict (Charades/ActivityNet) vs list (TimerFT)
    if isinstance(data, dict):
        # Charades/ActivityNet format: {video_id: {...}}
        ...
        # TimerFT format: [{video: "path", ...}, ...]
        for i, video_data in enumerate(data):
            video_path = video_data.get("video")
            if video_path is None:
                print(f"Warning: No video path found in entry {i}")
                continue

            # Extract video filename without extension for unique ID
        ...
        raise ValueError(f"Unsupported data format: {type(data)}. Expected dict or list.")
