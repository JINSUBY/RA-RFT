...
class TimeR1_Trainer_RARFT(Trainer):
    """
    Trainer for RA-RFT (Refusal-Aware Reinforcement Fine-Tuning) using Group Relative Policy Optimization (GRPO).

    This trainer extends the base GRPO method for video temporal grounding with refusal detection capabilities.
    It trains models to both accurately locate temporal segments and appropriately refuse unanswerable queries.

    Args:
        model (`Union[str, PreTrainedModel]`):
            Model to be trained. Can be either a model ID string or a PreTrainedModel object.
        reward_funcs (`Union[RewardFunc, list[RewardFunc]]`):
            Reward functions to be used for computing the rewards. Multiple reward functions are summed.
    ...
    def __init__(
        ...
        using_lora = peft_config is not None
        ...
        # Initialize Sentence-BERT model for refusal reward computation
        # Loaded with deepspeed.zero.Init(enabled=False) to avoid parameter partitioning
        # Uses deepspeed.zero.GatheredParameters during inference
        from main_rarft import init_sbert_model
        init_sbert_model()
    ...
    def make_conversation_video(self, example):
        if self.prompt_type == "rarft":
            prompt_text = QUESTION_TEMPLATE_RARFT.replace("[EVENT]", example["problem"])
        else:
            raise ValueError(f"Unsupported prompt_type: {self.prompt_type}. Only 'rarft' is supported for RA-RFT.")

        # Build video content dict with conditional video_start/video_end (Time-R1 style)
        video_content = {
            "type": "video",
        ...
        # Add video_start/video_end only if present (None values not added)
        if "video_start" in example:
            video_content["video_start"] = example["video_start"]
        if "video_end" in example:
            video_content["video_end"] = example["video_end"]
    ...
    def compute_loss(
        ...
        # Synchronize all GPUs at the start of iteration
        ...
        # Dynamic negative sampling: if task_type is refusable, select from pool
        # IMPORTANT: Use deterministic selection across all GPUs to avoid NCCL timeout
        if inputs[0].get('task_type') == 'refusable' and 'refusable_pool_json' in inputs[0]:
            ...
            # Deterministic sampling: all GPUs select the same index
            # Use global step as seed so it changes each forward pass
            seed = self.state.global_step if hasattr(self, 'state') else 0
            rng = random.Random(seed)

            selected_idx = rng.randint(0, pool_size - 1)

            # Update inputs[0] with selected query
            inputs[0]['problem'] = pool[selected_idx]['problem']
            inputs[0]['gt_answers'] = pool[selected_idx]['gt_answers']

            if torch.distributed.is_initialized() and torch.distributed.get_rank() == 0:
                print(f"[Dynamic Sampling] Selected index {selected_idx} from pool of {pool_size} queries (seed={seed})")
        ...
        # Log query, task type, and generated completions (answer part only)
        from main_rarft import extract_answer_content
        print(f"\n{'='*80}")
        print(f"Query: {inputs[0]['problem'][:100]}...")  # First 100 chars
        print(f"Task Type: {inputs[0]['task_type']}")
        print(f"Generated Completions ({self.num_generations}):")
        for i, completion in enumerate(completions):
            answer_content = extract_answer_content(completion)
            if answer_content:
                print(f"  [{i+1}] {answer_content[:150]}...")  # Show answer content only
            else:
                print(f"  [{i+1}] [No answer tag found]")
        print(f"{'='*80}\n")
        ...
        # Synchronize all GPUs before backward pass
        if torch.distributed.is_initialized():
            torch.distributed.barrier()
            if torch.distributed.get_rank() == 0:
                print(f"[Sync] Step {self.state.global_step}: All GPUs completed compute_loss")
